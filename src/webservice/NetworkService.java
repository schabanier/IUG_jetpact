...
public class NetworkService implements NetworkServiceInterface {
	...
	public void createAccount(Account newAccount, String newPassword)
		...
		try {
			...
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address
							+ "register?pseudo="
							+ URLEncoder.encode(newAccount.getPseudo(), "UTF-8")
							+ "&password="
							...
							+ URLEncoder.encode(newAccount.getFirstName(),
									"UTF-8")
							+ "&last_name="
							+ URLEncoder.encode(newAccount.getLastName(),
									"UTF-8")
							+ "&email="
							+ URLEncoder.encode(newAccount.getEMailAddress(),
									"UTF-8")));
				...
				if (inputStream != null) {
					...
					try {
						...
						// Else display error message
							...
							throw new IllegalFieldException(
									IllegalFieldException.PSEUDO,
									IllegalFieldException.REASON_VALUE_ALREADY_USED,
									newAccount.getPseudo());

						} else if (returnCode == ErrorCode.ILLEGAL_USE_OF_SPECIAL_CHARACTER) {
							currentPassword = null;
							throw new NetworkServiceException(
									"Illegal use of special character");
	...
	public Account authenticate(String pseudo, String password)
		...
		try {
			// make GET request to the given URL
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address + "login?pseudo="
							+ URLEncoder.encode(pseudo, "UTF-8") + "&password="
	...
	public void modifyEMailAddress(String emailAddress)
		...
		try {
			...
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address
							+ "modifyemail?pseudo="
							+ URLEncoder.encode(currentAccount.getPseudo(),
									"UTF-8") + "&password="
						...
						if (returnCode == NO_ERROR) {
							...
							for (int i = 0; i < arrayOfJsonTag.length(); i++) {
								...
								Tag tag = new Tag(tagjson.getString("tag_id"),
										...
										tagjson.getString("picture_name"));
								res.add(tag);
							}
						} else {
							throw new NotAuthenticatedException();
		...
		try {
			...
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address
							...
							+ URLEncoder.encode(currentPassword, "UTF-8")
							+ "&id="
							+ URLEncoder.encode(tag.getUid(), "UTF-8")
							+ "&object_name="
							+ URLEncoder.encode(tag.getObjectName(), "UTF-8")
							+ "&picture_name="
							+ URLEncoder.encode(tag.getObjectImageName(),
									"UTF-8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
	...
	public String downloadObjectImage(Tag tag)
		...
		try {
			...
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address
							+ "modifyobjectname?pseudo="
							+ URLEncoder.encode(currentAccount.getPseudo(),
									"UTF-8") + "&password="
							+ URLEncoder.encode(currentPassword, "UTF-8")
							+ "&id=" + URLEncoder.encode(tag.getUid(), "UTF-8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
				// receive response as inputStream
		...
		try {
			...
			HttpResponse httpResponse = executeRequest(new HttpGet(
					server_address
							...
							+ "&id=" + URLEncoder.encode(tag.getUid(), "UTF-8")
							+ "&new_object_name="
							+ URLEncoder.encode(newObjectName, "UTF-8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
				// receive response as inputStream
		...
		try {
			...
			HttpResponse httpResponse = client.execute(new HttpGet(
					server_address
							...
							+ URLEncoder.encode(profileName, "UTF-8")
							+ "&new_profile_name="
							+ URLEncoder.encode(newProfileName, "UTF-8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
				// receive response as inputStream
	...
	public Profile addTagToProfile(Profile profile, Tag tag)
		...
		try {
			...
			HttpResponse httpResponse = client.execute(new HttpGet(
					server_address
							+ "addtagtoprofile?pseudo="
							+ URLEncoder.encode(currentAccount.getPseudo(),
									"UTF-8") + "&password="
							+ URLEncoder.encode(currentPassword, "UTF-8")
							+ "&profile_name="
							+ URLEncoder.encode(profileName, "UTF-8") + "&id="
							+ URLEncoder.encode(tag.getUid(), "UTF-8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
	...
	public void removeProfile(Profile profile)
		...
		// changer aussi currentAccount ? -> changer visibilitÈ de l'attribut
		// Profils
		String profileName = profile.getName();
		// We first check the validity of the arguments to create the parameters
		if (!FieldVerifier.verifyName(profileName))
		...
		try {
			...
			HttpResponse httpResponse = client.execute(new HttpGet(
					server_address
							...
							+ URLEncoder.encode(currentAccount.getPseudo(),
									"UTF_8") + "&password="
							+ URLEncoder.encode(currentPassword, "UTF_8")));
			StatusLine statusLine = httpResponse.getStatusLine();
			int statusCode = statusLine.getStatusCode();
			if (statusCode == 200) {
				// receive response as inputStream
